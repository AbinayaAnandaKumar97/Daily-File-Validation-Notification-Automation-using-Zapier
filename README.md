...
Trigger (Scheduled / Manual)
        ↓
Code Step → Generate filenames dynamically
        ↓
Google Drive Search → Check each file
        ↓
Filter → Continue only if a file is missing
        ↓
Email Alert → Notify missing files
        ↓
Update Table → Log status and alert
